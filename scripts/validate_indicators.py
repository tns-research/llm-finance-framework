...
def validate_indicator_quality():
    ...
    if not parsed_dir.exists():
        print(
            "❌ No existing results found. Run the framework first with 'python -m src.main'"
        )
        return
    ...
    print(
        f"   Technical Indicators: {'✅ Enabled' if ENABLE_TECHNICAL_INDICATORS else '❌ Disabled'}"
    )
    print(
        f"   MACD, Stochastic, Bollinger Bands: {'✅ All enabled' if ENABLE_TECHNICAL_INDICATORS else '❌ All disabled'}"
    )
    ...
    print(
        f"   Close prices: ${features_with['close'].min():.2f} - ${features_with['close'].max():.2f}"
    )
    print(".1f")
    print(".2f")
...
def compare_indicator_distributions():
    ...
    if os.path.exists(features_path):
        ...
        if "macd_histogram" in features_df.columns:
            ...
            if len(macd_valid) > 0:
                print(
                    f"MACD Histogram - Mean: {macd_valid.mean():.3f}, Std: {macd_valid.std():.3f}"
                )
        ...
        if "stoch_k" in features_df.columns:
            ...
            if len(stoch_valid) > 0:
                print(
                    f"Stochastic %K - Mean: {stoch_valid.mean():.1f}, Std: {stoch_valid.std():.1f}"
                )
        ...
        if "bb_position" in features_df.columns:
            ...
            if len(bb_valid) > 0:
                print(
                    f"BB Position - Mean: {bb_valid.mean():.2f}, Std: {bb_valid.std():.2f}"
                )
    else:
