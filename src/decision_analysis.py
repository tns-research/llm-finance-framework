...
def analyze_decisions_after_outcomes(parsed_df: pd.DataFrame) -> dict:
    ...
    if len(df) == 0:
        return {
            "error": "No valid data for analysis (no rows with previous_return)"
        }
...
def create_decision_pattern_plots(parsed_df: pd.DataFrame, model_tag: str, output_dir: str):
    """
    ...
    fig.suptitle(f'Decision Pattern Analysis - {model_tag}', fontsize=16, fontweight='bold')
    ...
    ax1.bar(x - width/2, win_counts, width, label='After Wins', color='green', alpha=0.7)
    ax1.bar(x + width/2, loss_counts, width, label='After Losses', color='red', alpha=0.7)
    ...
    for i, (w, l) in enumerate(zip(win_counts, loss_counts)):
        ax1.text(i - width/2, w + 1, f'{w:.1f}%', ha='center', va='bottom', fontsize=9)
        ax1.text(i + width/2, l + 1, f'{l:.1f}%', ha='center', va='bottom', fontsize=9)
    ...
    for s in streak_range:
        if s in streak_decisions.index:
            ...
            buy_pcts.append(streak_decisions.loc[s].get('BUY', 0) / total * 100 if total > 0 else 0)
            hold_pcts.append(streak_decisions.loc[s].get('HOLD', 0) / total * 100 if total > 0 else 0)
            sell_pcts.append(streak_decisions.loc[s].get('SELL', 0) / total * 100 if total > 0 else 0)
        else:
            buy_pcts.append(0)
            hold_pcts.append(0)
            sell_pcts.append(0)
    ...
    ax4.bar(x_pos, sell_pcts, bottom=np.array(buy_pcts) + np.array(hold_pcts), 
...
def generate_pattern_analysis_report(parsed_df: pd.DataFrame, model_tag: str, output_path: str):
    ...
    if 'error' in decision_analysis:
        ...
        if decision_analysis['chi_square_test']:
            ...
            significance_text = "**statistically significant**" if chi_test['significant'] else "not statistically significant"
            report_lines.extend([
            ...
            if chi_test['significant']:
                ...
                report_lines.append("> The model's decision-making **is significantly influenced** by previous outcomes. ")
                report_lines.append("> This suggests the model adapts its strategy based on recent performance.")
            else:
                report_lines.append("> [!NOTE]")
                report_lines.append("> The model's decisions appear **independent** of previous outcomes. ")
                report_lines.append("> This suggests consistent strategy regardless of recent wins/losses.")
