...
def create_baseline_comparison_plot(
    comparison_df: pd.DataFrame, model_tag: str, output_path: str
):
    """
    ...
    ax4.axvline(
        x=50, color="red", linestyle="--", linewidth=1, alpha=0.7, label="50% line"
    )
...
def create_equity_curves_plot(
    ...
    random_representative_df = random_baseline(
        features_df, seed=42
    )  # Deterministic seed
    ...
    ax.set_title(
        f"Equity Curves Comparison - {model_tag}", fontsize=14, fontweight="bold"
    )
...
def run_baseline_analysis(
    ...
    llm_returns = (
        llm_parsed_df["strategy_return"] if llm_parsed_df is not None else None
    )
    llm_dates = llm_parsed_df["date"] if llm_parsed_df is not None else None
    create_equity_curves_plot(
        features_df, model_tag, equity_plot_path, llm_returns, llm_dates
    )
...
if __name__ == "__main__":
    ...
    run_baseline_analysis(
        features_path=features_path, output_dir=output_dir, model_tag="baselines_only"
    )
