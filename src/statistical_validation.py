...
def bootstrap_sharpe_comparison(
    ...
    p_value_two_sided = 2 * min(np.mean(diffs <= 0), np.mean(diffs >= 0))
    ...
    return {
        ...
        "effect_size": (
            round(observed_diff / np.std(diffs), 3) if np.std(diffs) > 0 else 0
        ),
...
def out_of_sample_validation(
    parsed_df: pd.DataFrame, split_date: str, min_train_periods: int = 100
) -> Dict:
    """
    ...
    if len(train_df) < min_train_periods:
        return {
            "error": f"Insufficient training data: {len(train_df)} < {min_train_periods}"
        }
    ...
    return_decay = None
    if (
        abs(train_metrics["total_return_pct"]) > 0.01
    ):  # Avoid division by very small numbers
        return_decay = (
            train_metrics["total_return_pct"] - test_metrics["total_return_pct"]
        ) / abs(train_metrics["total_return_pct"])

    # Overfitting detection
    overfitting_indicators = {
        "sharpe_decay_severe": sharpe_decay is not None
        and sharpe_decay > 0.5,  # >50% decay
        "return_decay_severe": return_decay is not None
        and return_decay > 0.7,  # >70% decay
        "test_performance_poor": test_metrics["sharpe_ratio"]
        < -0.5,  # Very poor test performance
        "high_train_overfitting": (
            train_metrics["sharpe_ratio"] > 1.0 and test_metrics["sharpe_ratio"] < 0.0
        ),
    }
    ...
    generalizes_well = (
        test_metrics["sharpe_ratio"] > 0.1  # Decent test Sharpe
        and not overall_overfitting_detected
    )
        ...
        "overfitting_detection": {
            **overfitting_indicators,
            ...
            "overfitting_severity": (
                "high"
                if sum(overfitting_indicators.values()) >= 2
                else "moderate" if sum(overfitting_indicators.values()) >= 1 else "low"
            ),
        },
...
def analyze_decision_effectiveness(parsed_df: pd.DataFrame) -> Dict:
    ...
    for decision in decisions:
        ...
        results["decision_distribution"][decision] = {
            "count": count,
            "percentage": (
                round(count / total_decisions * 100, 1) if total_decisions > 0 else 0
            ),
        }
    ...
    if results["decision_performance"]:
        ...
        best_decision = max(
            results["decision_performance"].keys(),
        ...
        results["risk_adjusted_analysis"] = {
            ...
            "best_excess_return": results["decision_performance"][best_decision][
                "excess_return_annualized"
            ],
            "worst_excess_return": results["decision_performance"][worst_decision][
                "excess_return_annualized"
            ],
            "decision_consistency": (
                "variable"
                if abs(
                    results["decision_performance"][best_decision][
                        "excess_return_annualized"
                    ]
                    - results["decision_performance"][worst_decision][
                        "excess_return_annualized"
                    ]
                )
                > 10
...
def evaluate_hold_decisions_dual_criteria(parsed_df: pd.DataFrame) -> Dict:
    ...
    for idx, row in hold_data.iterrows():
        ...
        vol_level = (
            ...
            if pd.notna(row["market_volatility_20d"])
            else 0
        )
    ...
    avg_relative_performance = (
        np.mean(relative_performance_scores) if relative_performance_scores else 0
    )
    ...
    for i, quiet_win in enumerate(quiet_markets):
        ...
        combined_score = quiet_weight * int(quiet_win) + context_weight * int(
            context_win
        )
    ...
    return {
        ...
        "hold_statistics": {
            ...
            "hold_percentage": (
                len(hold_decisions) / len(parsed_df) if len(parsed_df) > 0 else 0
            ),
            "avg_market_move_during_hold": (
                market_returns.abs().mean() if len(market_returns) > 0 else 0
            ),
            "hold_during_quiet_pct": (
                quiet_markets.mean() if len(quiet_markets) > 0 else 0
            ),
        },
        "summary": {
...
def generate_validation_summary(validation_results: Dict) -> Dict:
        ...
        if "error" not in oos:
            if oos["generalization_assessment"]["generalizes_well"]:
                summary["key_findings"].append(
                    "✅ Strategy generalizes well to out-of-sample data"
                )
            ...
            if oos["overfitting_detection"]["overall_overfitting_detected"]:
                ...
                summary["key_findings"].append(
                    f"🚨 {severity.capitalize()} overfitting detected"
                )

    # Bootstrap assessment
    bootstrap_key = (
        "bootstrap_vs_benchmark"
        if "bootstrap_vs_benchmark" in validation_results
        else "bootstrap_vs_index"
    )
    if bootstrap_key in validation_results:
        ...
        if bs["strategy_significantly_better_5pct"]:
            summary["key_findings"].append(
                f"✅ Strategy significantly outperforms {benchmark_name} (p={bs['p_value_strategy_better']})"
            )
        elif bs["strategy_significantly_worse_5pct"]:
            summary["key_findings"].append(
                f"❌ Strategy significantly underperforms {benchmark_name} (p={bs['p_value_strategy_worse']})"
            )
        else:
            summary["key_findings"].append(
                f"🤔 Strategy performance vs {benchmark_name} is not statistically significant"
            )
    ...
    if (
        summary["confidence_level"] == "high"
        and len([f for f in summary["key_findings"] if "✅" in f]) > 0
    ):
...
def calculate_var_and_stress_tests(
    returns: np.ndarray, dates: pd.Series = None
) -> Dict:
    """
    ...
    if len(returns) > 63:  # Need at least ~3 months for rolling VaR
        ...
        results["var_dates"] = (
            rolling_dates if rolling_dates else list(range(len(var_95)))
        )
...
def calculate_risk_attribution(
    strategy_returns: np.ndarray, market_returns: np.ndarray
) -> Dict:
    """
    ...
    return {
        ...
        "systematic_risk": (
            float(np.sqrt(systematic_risk)) if systematic_risk > 0 else 0
        ),
        "idiosyncratic_risk": (
            float(np.sqrt(idiosyncratic_risk)) if idiosyncratic_risk > 0 else 0
        ),
...
def analyze_market_regimes(parsed_df: pd.DataFrame) -> Dict:
    ...
    """
    if (
        "next_return_1d" not in parsed_df.columns
    ...
    if regime_results:
        best_regime = max(
            regime_results.keys(), key=lambda x: regime_results[x]["excess_return"]
        )
        worst_regime = min(
            regime_results.keys(), key=lambda x: regime_results[x]["excess_return"]
        )
        ...
        regime_results["_summary"] = {
            ...
            "performance_range": abs(
                regime_results[best_regime]["excess_return"]
...
def print_validation_report(validation_results: Dict, model_tag: str):
    ...
    if "dataset_info" in validation_results:
        ...
        print(
            f"Strategy Return: {ds['total_strategy_return']:.2f}% | Index Return: {ds['total_index_return']:.2f}%"
        )
    ...
    if "out_of_sample_validation" in validation_results:
        ...
        if "error" not in oos:
            print(f"\nOUT-OF-SAMPLE VALIDATION:")
            print(
                f"  Train Period: {oos['train_period']['sharpe_ratio']:.3f} Sharpe ({oos['train_period']['n_periods']} periods)"
            )
            print(
                f"  Test Period:  {oos['test_period']['sharpe_ratio']:.3f} Sharpe ({oos['test_period']['n_periods']} periods)"
            )
            ...
            generalizes = (
                "YES" if oos["generalization_assessment"]["generalizes_well"] else "NO"
            )
