...
def build_summary_row(row: pd.Series) -> str:
    ...
    if (
        ...
        and "rsi_14" in row.index
        and not pd.isna(row["rsi_14"])
    ):
        rsi_level = (
            "overbought"
            if row["rsi_14"] > config.RSI_OVERBOUGHT
            else "oversold" if row["rsi_14"] < config.RSI_OVERSOLD else "neutral"
        )
    ...
    if (
        config.ENABLE_TECHNICAL_INDICATORS
        and "macd_histogram" in row.index
        and not pd.isna(row["macd_histogram"])
    ):
        macd_signal = "bullish" if row["macd_histogram"] > 0 else "bearish"
        summary_parts.append(
            f"MACD histogram is {row['macd_histogram']:.3f} ({macd_signal})."
        )
    ...
    if (
        config.ENABLE_TECHNICAL_INDICATORS
        and "stoch_k" in row.index
        and not pd.isna(row["stoch_k"])
    ):
        stoch_level = (
            "overbought"
            if row["stoch_k"] > config.STOCH_OVERBOUGHT
            else "oversold" if row["stoch_k"] < config.STOCH_OVERSOLD else "neutral"
        )
    ...
    if (
        config.ENABLE_TECHNICAL_INDICATORS
        and "bb_position" in row.index
        and not pd.isna(row["bb_position"])
    ):
        bb_pos = row["bb_position"]
        bb_desc = (
            "upper band"
            if bb_pos > 0.8
            else "lower band" if bb_pos < 0.2 else "middle range"
        )
...
def row_to_prompt(row: pd.Series) -> str:
    ...
    if config.ENABLE_TECHNICAL_INDICATORS:
        ...
        if all(
            f"macd_hist_lag_{k}" in row.index
            for k in range(1, config.PAST_RET_LAGS + 1)
        ):
            past_macd = [
                row[f"macd_hist_lag_{k}"] for k in range(config.PAST_RET_LAGS, 0, -1)
            ]
        ...
        if all(
            f"stoch_k_lag_{k}" in row.index for k in range(1, config.PAST_RET_LAGS + 1)
        ):
            past_stoch = [
                row[f"stoch_k_lag_{k}"] for k in range(config.PAST_RET_LAGS, 0, -1)
            ]
        ...
        if all(
            f"bb_position_lag_{k}" in row.index
            for k in range(1, config.PAST_RET_LAGS + 1)
        ):
            past_bb = [
                row[f"bb_position_lag_{k}"] for k in range(config.PAST_RET_LAGS, 0, -1)
            ]
    ...
    if (
        config.ENABLE_TECHNICAL_INDICATORS
        and "macd_line" in row.index
        and not pd.isna(row["macd_line"])
    ):
        text_parts.append(
