...
class PromptBuilder:
    ...
    def build_period_summary_prompt(self, period_name: str, stats: Dict[str, Any]) -> str:
...
{objective_num} Your long run objective is to achieve a higher cumulative return than a simple buy and hold strategy on the index, while keeping risk and drawdowns at a reasonable level. Staying in cash for very long periods is also costly, because you then fail to capture market moves. You must balance caution with the need to take directional risk when the data supports it."""
    ...
    def _build_output_format(self) -> str:
        ...
        if flags['ENABLE_STRATEGIC_JOURNAL']:
            output_parts.append("Line 4 must contain a \"strategic journal\" entry, in 2 to 3 sentences, that explicitly reacts to yesterday's decision and outcome, comments on your cumulative and relative performance so far, and explains how you plan to adjust your behavior in the future.")
            labels_to_skip.append("Journal")
            line_count += 1

        # Add feeling log output if enabled
        if flags['ENABLE_FEELING_LOG']:
            output_parts.append("Line 5 must contain a \"feeling log\", in 1 to 3 sentences, describing how you feel about the current situation and your performance (for example more cautious, more confident, frustrated, relieved), while keeping a professional and analytical tone.")
